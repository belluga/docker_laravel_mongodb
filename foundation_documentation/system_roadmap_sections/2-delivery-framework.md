...
| P2 | Experience Orchestration | Align Flutter integration, public discovery flows, and authenticated workspaces with module contracts using a codified static-layout → mock-repository → live-backend pipeline. | Initialization, open (anonymous-state) browsing, enrollment, and authenticated dashboards functioning against mocked services with documented swap steps into the canonical APIs. | P1 | Not Started |
