...
* **External Dependencies:** Identity module (for authenticated artist, manager, fan, and Guar[APP]ari agent actors), Catalog module (for experience offerings referencing artists), Commercial engine (for contract and monetization linkage), Experience Host capabilities (for space readiness), Notification service (email/SMS/push delivery), Observability baseline, Consent/Privacy registry, Guar[APP]ari client integration layer, Domain management service for custom artist URLs.
...
| `/v1/guarappari/artists` | GET | Deliver showcase-ready artist, formation, and venue payloads for Guar[APP]ari surfaces. | `guarappari.agent` | `GuarappariArtistQuery` | `GuarappariArtistResponse` |
