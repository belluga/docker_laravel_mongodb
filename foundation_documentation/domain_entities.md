...
- **Artist Profile**: Individual or collective performer attached to events (musicians, chefs-in-residence, mixologists, guest speakers). Artist profiles capture genres, rider requirements, booking windows, compensation structures, collaborative history across hosts and guides, power the Guar[APP]ari showcase surfaces without duplicating data, and provision exclusive microsites served on custom domains with single-artist tenant isolation.
