...
- **Artist Profile**: Individual or collective performer attached to events (musicians, chefs-in-residence, mixologists, guest speakers). Artist profiles capture genres, rider requirements, booking windows, compensation structures, collaborative history across hosts and guides, power the Belluga Now (Guar[APP]ari) discovery surfaces without duplicating data, and provision exclusive microsites served on custom domains with single-artist tenant isolation.
...
- **Invitation Thread**: Canonical invitation construct linking hosts, invitees, dispatch channels, RSVP outcomes, and propagation metrics across Belluga and Guar[APP]ari surfaces.
